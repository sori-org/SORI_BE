...
def get_or_create_kakao_user(db: Session, kakao_user_data: dict, access_token: str):
    ...
    account = db.query(Accounts).filter_by(kakao_id=kakao_id, is_deleted=False).first()

    if account:
        # 유저도 찾기
        user = db.query(Users).filter_by(account_id=account.account_id).first()
        return user, False  # 이미 가입된 유저

    # 2. 신규 계정 생성
    try:
        new_account = Accounts(kakao_id=kakao_id)
        db.add(new_account)
